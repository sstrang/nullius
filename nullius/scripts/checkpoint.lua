...
local function get_stored_fluid(force, fluid)
  ...
  -- Factorio 2.1.7 removed LuaEntity::fluidbox; the segment methods moved
  -- onto LuaEntity itself.
  local measured = {}  -- measured[id] if fluid segment id was scanned
  local total = 0
  for _, tank in pairs(tanks) do
    local segment_id = tank.get_fluid_segment_id(1)
    if segment_id ~= nil and not measured[segment_id] then
      measured[segment_id] = true
      total = total + (tank.get_fluid_segment_contents(1)[fluid] or 0)
    end
  end
